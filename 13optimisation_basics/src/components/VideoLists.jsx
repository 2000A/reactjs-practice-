...
//but if we want to render our videos on first render just like youtube we need to put our side effect in useEffect
  ...
  const play = useCallback(() => console.log("playing", videos.title),[])
  const pause= useCallback(() => console.log("pause", videos.title),[])
  return (
    <>
...
// if doing component memoization then useMemo or useCallback will come into play 
//() => console.log("playing", videos.title) as when videoLists will render again 
// onPlay={() => console.log("playing", videos.title)}
// onPause={() => console.log(videos.title, "paused")}
//the function we are passing as prop will get copied meaning this prop will change and our memoized video component will also render (see comment in video.jsx)